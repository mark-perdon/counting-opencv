...
namespace FAV1
{
  ...
  void PeopleCounting_on_mouse(int evt, int x, int y, int flag, void* param)
  {
    if(!use_roi)
      return;
    ...
    if(evt == CV_EVENT_MOUSEMOVE && startDraw)
    {
      ...
      cvShowImage("PeopleCounting", img_input2);
      cvReleaseImage(&img_input2);
    }
  }
}

PeopleCounting::PeopleCounting(): firstTime(true), showOutput(true), key(0), countAB(0), countBA(0), showAB(0)
{
  std::cout << "PeopleCounting()" << std::endl;
}
...
void PeopleCounting::setInput(const cv::Mat &i)
{
  //i.copyTo(img_input);
  img_input = i;
}

void PeopleCounting::setTracks(const cvb::CvTracks &t)
{
  tracks = t;
}

VehiclePosition PeopleCounting::getVehiclePosition(const CvPoint2D64f centroid)
{
...
void PeopleCounting::process()
{
  ...
  if(FAV1::use_roi == true && FAV1::roi_defined == false && firstTime == true)
  {
    do
    {
	  ...
	  imshow("PeopleCounting", img_input);
      FAV1::img_input1 = new IplImage(img_input);
	  cvSetMouseCallback("PeopleCounting", FAV1::PeopleCounting_on_mouse, NULL);
	  //cvResizeWindow("PeopleCounting", 640, 640);
	  key = cvWaitKey(0);
...
//  cvSetWindowProperty("VehicleCounting", CV_WND_PROP_FULLSCREEN, CV_WINDOW_FULLSCREEN);
  ...
  cv::Point fpsPoint(10, 130);
  cv::putText(img_input, "FPS:", fpsPoint, cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255, 255, 255));
  ...
  if(ROI_OK)
  {
    ...
    {
      if(!firstTime)
        cv::putText(img_input, "HORIZONTAL", cv::Point(10,15), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(209,11,35));
      laneOrientation = LO_HORIZONTAL;

	  cv::putText(img_input, "A", cv::Point(FAV1::roi_x0 - 32, FAV1::roi_y0), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(236, 93, 143));
	  cv::putText(img_input, "B", cv::Point(FAV1::roi_x0 + 12, FAV1::roi_y0), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(236, 93, 143));
    }

    if(abs(FAV1::roi_x0 - FAV1::roi_x1) > abs(FAV1::roi_y0 - FAV1::roi_y1))
    {
      if(!firstTime)
		  cv::putText(img_input, "VERTICAL", cv::Point(10, 15), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(209, 11, 35));
      laneOrientation = LO_VERTICAL;
	  cv::putText(img_input, "A", cv::Point(FAV1::roi_x0, FAV1::roi_y0 + 22), cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(236, 93,143));
	  cv::putText(img_input, "B", cv::Point(FAV1::roi_x0, FAV1::roi_y0 - 12), cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(236, 93, 143));
    }
  }
  ...
  if(showAB == 0)
  {
	  cv::putText(img_input, countABstr, cv::Point(10, img_h - 20), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(0, 0, 255), 1);
	  cv::putText(img_input, countBAstr, cv::Point(10, img_h - 8), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(0, 0, 255), 1);
  }
  
  if(showAB == 1)
	  cv::putText(img_input, countABstr, cv::Point(10, img_h - 8), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(0, 0, 255), 1);
  
  if(showAB == 2)
	  cv::putText(img_input, countBAstr, cv::Point(10, img_h - 8), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(0, 0, 255), 1);

  if(showOutput)
    cv::imshow("PeopleCounting", img_input);
...
void PeopleCounting::saveConfig()
{
  CvFileStorage* fs = cvOpenFileStorage("config/VehicleCounting.xml", 0, CV_STORAGE_WRITE);
...
void PeopleCounting::loadConfig()
{
  CvFileStorage* fs = cvOpenFileStorage("config/VehicleCounting.xml", 0, CV_STORAGE_READ);
